...
static bool	prepare_ipv4_data(t_server *server, struct sockaddr_in *addr)
{
	...
	if (server->sock < 0)
	{
		error(INVALID_SOCKET_IPV4, "[LOG !] Can't create socket using ipv4");
		return (false);
	}
...
static bool	bind_socket_ipv4(t_server *server)
{
	...
	{
		error(INVALID_BIND, "[LOG !] Can't bind socket to address using ipv4");
		return (false);
	}
...
static bool	prepare_ipv6_data(t_server *server, struct sockaddr_in6 *addr)
{
	...
	if (server->sock < 0)
	{
		error(INVALID_SOCKET_IPV6, "[LOG !] Can't create socket using ipv6");
		return (false);
	}
...
static bool	bind_socket_ipv6(t_server *server)
{
	...
	if (bind(server->sock, (struct sockaddr*)&addr, sizeof(struct sockaddr)) < 0)
	{
		error(INVALID_BIND_IPV6, "[LOG !] Can't bind socket to address using ipv6");
		return (false);
	}
